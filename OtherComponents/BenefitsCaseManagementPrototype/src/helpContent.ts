...
export const helpContent: Record<HelpContextId, HelpContent> = {
  ...
  inbox: {
    ...
    relatedActions: ['Open Case', 'Assign to Me', 'Mark Priority', 'View Flow'],
    externalReferences: commonReferences,
  },
  detail: {
    screenId: 'detail',
    ...
    workerGuidance: [
      ...
      'Timeline / Audit shows the linked Maestro instance flow for the selected application.',
    ],
  ...
  actions: {
    ...
    category: 'Case Review',
    title: 'Actions Help',
    summary: 'Use Actions to open assigned UiPath Action Center tasks for the selected case.',
    workerGuidance: [
      'Confirm the case is linked to a Maestro instance.',
      'Refresh actions to load assigned tasks.',
      'Open a task when you need the full Action Center view.',
      'Return to the Actions tab after closing the task overlay.',
    ],
    checklist: ['Check the Maestro instance.', 'Refresh actions.', 'Open assigned tasks.', 'Return to the Actions tab.'],
    keyTerms: [
      { term: 'Action Center Task', definition: 'A human task created by Maestro and surfaced inside the case management UI.' },
      { term: 'Maestro Instance', definition: 'The running process instance associated with this case.' },
    ],
    relatedActions: ['Refresh Actions', 'Open Task', 'Open Maestro Instance'],
    externalReferences: commonReferences,
  },
  application: {
    ...
    summary: 'Use Application to inspect the submitted SNAP application and responses.',
    workerGuidance: [
    ...
    checklist: ['Review application PDF.', 'Check applicant responses.', 'Review household members.', 'Flag missing fields when needed.'],
    keyTerms: [
  ...
  interview: {
    ...
    summary: 'Use this tab to review live interview status, missing fields, and assigned interview tasks.',
    workerGuidance: [
      'Confirm whether interview details have been recorded in Data Fabric.',
      'Track missing fields before budget and notice steps.',
      'Open assigned interview tasks when Maestro has created them.',
    ],
    checklist: ['Review interview status.', 'Check missing fields.', 'Open assigned interview tasks.', 'Return to the case workspace.'],
    keyTerms: [
      { term: 'Missing Information', definition: 'Unresolved fields or evidence that block case completion.' },
      { term: 'Applicant Response Status', definition: 'Recorded state showing whether the applicant has responded.' },
    ],
    relatedActions: ['Open Interview Task', 'Refresh Actions', 'Open Maestro Instance'],
    externalReferences: commonReferences,
  },
  documents: {
    screenId: 'documents',
    ...
    workerGuidance: [
      ...
      'If the document is blurry or incomplete, request a replacement before final review.',
    ],
    checklist: [
  ...
  budget: {
    ...
    category: 'Budget',
    title: 'Budget Help',
    summary: 'Use Budget to review readiness, income used, calculation status, and worker notes.',
    workerGuidance: [
      'Confirm application, income, and clearance are ready.',
      'Review the calculated amount after the Maestro budget step completes.',
      'Mark the budget reviewed when the result is ready for the next action.',
    ],
    checklist: ['Review readiness checklist.', 'Create budget.', 'Review calculated amount.', 'Mark budget reviewed.'],
    keyTerms: [
  ...
  timeline: {
    ...
    summary: 'Use Timeline / Audit to review the exact Maestro instance flow and live runtime position for the selected application.',
    workerGuidance: [
      'Confirm the selected application has a linked Maestro instance.',
      'Watch the live status strip or refresh manually after workflow progress changes.',
      'Review incidents only when they are returned for matching BPMN elements.',
    ],
    checklist: ['Open a case with a Maestro instance.', 'Review the BPMN diagram.', 'Refresh if the process has moved forward.'],
    keyTerms: [
      { term: 'Maestro Instance Flow', definition: 'The BPMN diagram returned for the selected Maestro process instance.' },
      { term: 'BPMN Element', definition: 'A task, event, or gateway in the Maestro flow diagram.' },
    ],
    relatedActions: ['Refresh', 'Open Maestro'],
    externalReferences: commonReferences,
  },
  json: {
    screenId: 'json',
  ...
  assignment: {
    screenId: 'assignment',
    category: 'Dashboard & Metrics',
    title: 'Assignment Dashboard Help',
    summary: 'Use the assignment dashboard to review Maestro-created tasks, start worker PTO updates, and run supervisor single-user task reassignment.',
    workerGuidance: [
      'Review every local Maestro task and its assigned team or user.',
      'Filter tasks by Ready, Queued, or Completed before reassigning work.',
      'Open PTO Calendar to start the Worker PTO process for a worker email and date range.',
      'Switch between the two prototype personas only: Case Worker and Supervisor.',
      'Use the assignment choice dropdown, then confirm the assignee email for a loaded Action Center task ID.',
      'Open My Team to review local case worker roster placeholders.',
    ],
    checklist: ['Filter to the right task status.', 'Review PTO conflicts.', 'Set PTO if a worker is unavailable.', 'Review My Team.', 'Change a task user.', 'Open the case if more context is needed.'],
    keyTerms: [
      { term: 'Maestro Task Assignment', definition: 'A local row representing a task created by the IES Maestro process or its subprocesses.' },
      { term: 'PTO Calendar', definition: 'A worker availability calendar that starts the Worker PTO process and keeps local demo state updated.' },
      { term: 'My Team', definition: 'A local supervisor roster with placeholder team-member creation.' },
      { term: 'Change Assignment', definition: 'Supervisor action that starts the Assign case worker to a task process with user email and task ID.' },
    ],
    relatedActions: ['Set PTO', 'Change Assignment', 'Add a new Team Member', 'Open Case'],
    externalReferences: commonReferences,
  },
  testing: {
    screenId: 'testing',
  ...
  helpCenter: {
    ...
    workerGuidance: [
      'Use search to find terms such as reason code, document, or clearance.',
...
export const glossary = [
  ...
  { term: 'Maestro Instance Flow', definition: 'The BPMN diagram returned for a linked Maestro process instance.' },
...
export const roleHelp: Record<Role, string[]> = {
  ...
  'Eligibility Specialist': [
    ...
    'Use Budget, Forms & Notices, and Timeline / Audit when the case is ready.',
  ],
  Supervisor: [
  ...
  Auditor: [
    'Focus on read-only review, Maestro flow state, status changes, and raw JSON inspection.',
...
export const demoCoachTips: Partial<Record<HelpContextId, string[]>> = {
  ...
  inbox: ['Start here: open MYB-1004 for the Michael Motorist SNAP review.', 'Use exception filters to show a living queue.'],
  ...
  timeline: ['End by viewing Timeline / Audit to show the live Maestro instance flow.'],
  operations: ['Open Operations Dashboard to review county and region bottlenecks, then open IES SNAP Dash.'],
  assignment: ['Open Assignment Dashboard to review PTO, live task assignment controls, and My Team.'],
  actions: ['Open the Actions tab inside a case to refresh, open, or complete pending UiPath tasks.'],
